...
threshold = 0.8;
options = odeset('Events',@(t,Y) myEventsFcn_3(t,Y,ntot,threshold)); % For each generation, stop simulations when activator/inhibitor reaches the threshold
Vbirth = NaN(50,50);
amplitude = NaN(50,50);
for i=1:50
    for j=1:i-1
        ...
        if generation==10
            Vbirth(i,j) = sum(Y(1,ntot+1:2*ntot)'.*L)/rho;
            amplitude(i,j) = Y(end,ntot+3)/Y(end,ntot+4)-min(Y(:,ntot+3)./Y(:,ntot+4));
        end
    end
end