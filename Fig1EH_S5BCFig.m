...
for i = 1:150
    ...
    if generation==15
        threshold(i) = Y(end,ntot+3)/Y(end,ntot+4); % Then, determine the corresponding division threshold and perform formal simulations
        options1 = odeset('Events',@(t,Y) myEventsFcn_3(t,Y,ntot,threshold(i))); % For each generation, stop simulations when activator/inhibitor reaches the threshold
        ...
        if generation1==15
            ...
            amplitude(i) = Y(end,ntot+3)/Y(end,ntot+4)-min(Y(:,ntot+3)./Y(:,ntot+4));
        end
    end
end