...
V_all = [];
tlast = 0;
theta = threshold+dd/3*randn(genmax,1); % \theta~N(threshold,(dd/3)^2)
for generation = 1:genmax
    if theta(generation)<threshold-dd || theta(generation)>threshold+dd
        theta(generation) = threshold;
    end
end
for generation = 1:genmax
    ...
    Y0 = Y(end,:)'/2;
    tlast = t_all(end);
    V_birth(generation) = V(1);
    T_generation(generation) = t(end);
    Vd(generation) = V(end);
    if t(end)>=tmax
        break;
    end
