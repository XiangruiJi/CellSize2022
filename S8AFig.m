...
for i = 1:150
    ...
    if generation==15
        threshold(i) = rho*Y(end,ntot+3)/sum(Y(end,ntot+1:2*ntot)'.*L); % Then, determine the corresponding division threshold and perform formal simulations
        options1 = odeset('Events',@(t,Y) myEventsFcn_9(t,Y,ntot,rho,L,threshold(i)));
