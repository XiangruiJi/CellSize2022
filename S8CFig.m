...
for i = 1:150
    ...
    options = odeset('Events',@(t,Y) myEventsFcn_1(t,Y,Mmax,ntot,L)); %  First, let the cell reach a steady state with the mass at division equal to Mmax
    Y0 = [m_0;p_0];
    for generation = 1:15
        [t,Y,~,~,~] = ode23(@(t,Y) odefun(t,Y,ntot,rho,a,g,L,Gamma_n,K_n,v_n,tau_m,Gamma_r,K_r,v_r,tau_p),[0,tmax],Y0,options); % something wrong with ode45
        Y0 = Y(end,:)'/2;
        if t(end)>=tmax
            break;
        end
    ...
    if generation==15
        threshold(i) = rho*Y(end,ntot+4)/sum(Y(end,ntot+1:2*ntot)'.*L); % Then, determine the corresponding division threshold and perform formal simulations
        options1 = odeset('Events',@(t,Y) myEventsFcn_10(t,Y,ntot,rho,L,threshold(i)));
