function [meanV,CVV,V_birth] = stochastictrace2(thetabar,dd,genmax,m_0,p_0,tmax,ntot,rho,a,g,L,Gamma_n,K_n,v_n,tau_m,Gamma_r,K_r,v_r,tau_p,gx)
% simulate the cell cycle to a steady state using the deterministic model
options = odeset('Events',@(t,Y) myEventsFcn_11(t,Y,ntot,thetabar,gx));
Y0 = [m_0;p_0];
for generation = 1:15
...
if generation==15
    ...
    for generation = 1:genmax
        options = odeset('Events',@(t,Y) myEventsFcn_11(t,Y,ntot,theta(generation),gx));
        [t,Y,~,~,~] = ode45(@(t,Y) odefun(t,Y,ntot,rho,a,g,L,Gamma_n,K_n,v_n,tau_m,Gamma_r,K_r,v_r,tau_p),[0,tmax],Y0,options);
        Y0 = Y(end,:)'/2;
